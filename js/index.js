...
// map.setView([39.3520336,16.2410621], 12);
			...
			if (photo.geo_is_public) {
				...
				photos.push({
					...
					caption : '	<table width="100%"> ' +
							  '		<tr> ' +
										'<td>' + photo.title + '</td>' +
										'<td style="text-align: right"><a href="' + photo.url_z + '" target="_blank">Visualizza</a></td>' +
									'</tr>' +
									'<tr>' +
										'<td>Data: ' + d.getDate() + ' ' + monthNames[d.getMonth()] + ' ' + d.getFullYear() + '</td>' +
										'<td style="text-align: right"><a href="http://maps.google.com/maps?f=d&hl=it&daddr=' + pos[0] + ',' + pos[1] + '&ie=UTF8&0&om=0&output=kml" target="_blank">Raggiungi il sito</a></td>' +
									'</tr>' +
								'</table>',
